...
// function to run query to get detail of matrimonial members
    ...
    public function getMessages()
    {
        ...
        $builderMsg->select('mm.id,mm.user_id,msg,first_name,middle_name,last_name ,time,check_msg,deleted');
        $builderMsg->where('mm.contact_user_id', $id);
        $builderMsg->where('mm.deleted', null);
    ...
    {
        $builder = $this->msgs_builder;
        $builder->set(['deleted' => date('Y-m-d')]);
        $builder->where('id', $id);
        $builder->update();
    }
    ...
    public function getCountNotification($contact_user_id)
    {
        ...
        $this->msgs_builder->where("check_msg", 0);
